...
-- 8. Create RLS policies (handle duplicates with exception handling)
DO $$
BEGIN
    -- Drop existing policies first to avoid conflicts
    DROP POLICY IF EXISTS "Users can manage their own books" ON public.user_books;
    DROP POLICY IF EXISTS "Users can view others books" ON public.user_books;
...
-- Migration completed
SELECT 'Final database cleanup migration completed successfully' as status;